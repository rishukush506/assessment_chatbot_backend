...
def get_user_chats_2(user_id: str):
    """
    Fetch chat history for a given user_id
    """
    chats = list(messages_2.find({"user_id": user_id}))  # Exclude _id
    return chats
...
def get_user_persona_2(user_id: str):
    """
    Fetch persona for a given user_id
    """
    persona = personas_2.find({"user_id": user_id})
    return persona
...
# results.append(get_user_chats_2("user88"))
# results.append(get_user_chats_2("user89"))
...
# print(df.shape)
# df.to_excel(f"chat_history_all_users.xlsx", index=False)

results_persona = []
results_persona.append(get_user_persona_2("user88"))
results_persona.append(get_user_persona_2("user89"))
...
df_persona.to_excel(f"persona_all_users.xlsx", index=False)
